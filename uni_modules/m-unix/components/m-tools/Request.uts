...
import { uploadFileRequest } from './Upload.uts'
...
export function publicRequest<T = any>(options: RequestOptions): Promise<ApiResponse<T>> {
	return request<T>({ ...options, withToken: false, redirectOnUnauthorized: false })
}
...
export function silentRequest<T = any>(options: RequestOptions): Promise<ApiResponse<T>> {
	return request<T>({ ...options, showError: false, redirectOnUnauthorized: false })
}
...
export function loadingRequest<T = any>(options: RequestOptions, loadingText?: string): Promise<ApiResponse<T>> {
	return request<T>({ ...options, showLoading: true, loadingText: loadingText || '加载中...' })
}
...
export const http = {
	...
	/** 公开请求（不需要登录） */
	public<T = any>(options: RequestOptions): Promise<ApiResponse<T>> {
		return publicRequest<T>(options)
	},

	/** 静默请求（不显示提示） */
	silent<T = any>(options: RequestOptions): Promise<ApiResponse<T>> {
		return silentRequest<T>(options)
	},

	/** 带加载提示的请求 */
	loading<T = any>(options: RequestOptions, loadingText?: string): Promise<ApiResponse<T>> {
		return loadingRequest<T>(options, loadingText)
	},

	/** multipart 上传单个文件（实现见 Upload.uts） */
	upload<T = any>(options: any): Promise<any> {

...
open class GenComponentsTopNavBarTopNavBar : VueComponent {
    ...
    companion object {
                        ...
                        _cE("view", _uM("key" to 1, "class" to "navbar", "style" to _nS(navBarStyle.value)), _uA(
                            _cE("view", _uM("class" to "back-btn", "style" to _nS(_uM("visibility" to if (_ctx.showBack) {
                                ...
                                if (isTrue(_ctx.showBack)) {
                                    _cV(_component_rice_icon, _uM("key" to 0, "name" to "arrow-left", "size" to "35rpx", "color" to "#333", "class" to "icon", "onClick" to handleBack))
                                } else {
                                    _cC("v-if", true)
                                }
                            ), 4),
                                    ...
                                    if (isTrue(_ctx.showCapsule)) {
                                        _cE("view", _uM("key" to 0, "onClick" to fun(){
                                            ...
                                            if (isTrue(_ctx.isIcon)) {
                                                _cV(_component_rice_icon, _uM("key" to 0, "name" to _ctx.Icon, "size" to "26", "color" to _ctx.iconColor), null, 8, _uA(
                                                    "name",
                                                    "color"
                                                ))
                                            } else {
                        ...
                        _cE(Fragment, _uM("key" to 2), _uA(
                            ...
                            _cE("view", _uM("class" to "navbar", "style" to _nS(_uM("height" to (unref(navBarHeight) + "px"), "backgroundColor" to _ctx.backgroundColor))), _uA(
                                _cE("view", _uM("class" to "back-btn", "style" to _nS(_uM("visibility" to if (_ctx.showBack) {
                                    ...
                                    if (isTrue(_ctx.showBack)) {
                                        _cV(_component_rice_icon, _uM("key" to 0, "name" to "arrow-left", "size" to "40rpx", "class" to "icon", "onClick" to handleBack))
                                    } else {
                                        _cC("v-if", true)
                                    }
                                ), 4),
        ...
        val styles0: Map<String, Map<String, Map<String, Any>>>
            get() {
                return _uM("navbar" to _pS(_uM("display" to "flex", "flexDirection" to "row", "justifyContent" to "center", "alignItems" to "center", "boxSizing" to "border-box", "position" to "relative")), "back-btn" to _pS(_uM("display" to "flex", "alignItems" to "center", "width" to "70rpx", "height" to "40rpx", "zIndex" to 2, "justifyContent" to "center", "flexShrink" to 0)), "title" to _pS(_uM("flexGrow" to 1, "flexShrink" to 1, "flexBasis" to "0%", "textAlign" to "center", "fontWeight" to "bold", "fontSize" to "30rpx", "overflow" to "hidden", "textOverflow" to "ellipsis", "whiteSpace" to "nowrap", "paddingTop" to 0, "paddingRight" to 0, "paddingBottom" to 0, "paddingLeft" to 0, "display" to "flex", "flexDirection" to "row", "alignItems" to "center", "justifyContent" to "center", "position" to "absolute", "left" to 0, "right" to 0, "width" to "100%", "zIndex" to 1)), "capsule" to _pS(_uM("display" to "flex", "alignItems" to "center", "justifyContent" to "flex-end", "flexShrink" to 0, "width" to "70rpx", "zIndex" to 2)), "capsule-item" to _pS(_uM("display" to "flex", "justifyContent" to "center", "alignItems" to "center")), "icon" to _pS(_uM("width" to "40rpx", "height" to "40rpx")))
            }

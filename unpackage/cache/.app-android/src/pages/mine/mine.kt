...
open class GenPagesMineMine : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesMineMine) -> Any? = fun(__props): Any? {
            ...
            val isWechatEnv = computed<Boolean>(fun(): Boolean {
                return isWechat()
            }
            )
            ...
            val isLoginState = computed<Boolean>(fun(): Boolean {
                return !checkToken()
            }
            )
            ...
            val userLoginByOpenid = fun(codes: String): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        val res = await(login(_uO("xcxCode" to codes, "isLogin" to "0")))
                        if (res.code == 200) {
            ...
            val code = ref<String>("")
            val getCode = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        uni_login(LoginOptions(success = fun(res){
                            code.value = res.code
                            if (wxGetPhoneLogin.value == "1") {
                                val params: UTSJSONObject = _uO("__\$originalPosition" to UTSSourceMapPosition("params", "pages/mine/mine.uvue", 103, 12), "isLogin" to "1", "xcxCode" to code.value)
                                login(params).then(fun(res){
                                    if (res.code == 200) {
            ...
            val handleLogout = fun(){
                clearToken()
                uni_showToast(ShowToastOptions(title = "退出登录成功", icon = "none"))
                uni_reLaunch(ReLaunchOptions(url = "/pages/card/card"))
            }
            val cardListSum = ref<CardListSumData>(CardListSumData(all = 0, inUse = 0, inNotUse = 0))
            val getCardListSum = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        try {
                            val res = await(queryCardListSum())
                            if (res.code === 200) {
                    ...
                    _cE("view", _uM("class" to "container"), _uA(
                        _cE("view", _uM("class" to "card-info mb-24"), _uA(
                            _cE("view", _uM("class" to "persion-card"), _uA(
                                _cE("view", _uM("class" to "persion-card-item"), _uA(
                                    _cE("text", _uM("class" to "persion-card-item-title"), "我的卡片"),
                                    _cE("text", _uM("class" to "persion-card-item-content"), _tD(if (cardListSum.value.all != null) {
                                        cardListSum.value.all
                                    } else {
                                        0
                                    }
                                    ), 1)
                                ...
                                _cE("view", _uM("class" to "persion-card-item"), _uA(
                                    ...
                                    _cE("text", _uM("class" to "persion-card-item-content"), _tD(if (cardListSum.value.inUse != null) {
                                        cardListSum.value.inUse
                                    } else {
                                        0
                                    }
                                    ), 1)
                                ...
                                _cE("view", _uM("class" to "persion-card-item"), _uA(
                                    ...
                                    _cE("text", _uM("class" to "persion-card-item-content"), _tD(if (cardListSum.value.inNotUse != null) {
                                        cardListSum.value.inNotUse
                                    } else {
                                        0
                                    }
                                    ), 1)
                        ...
                        _cE("view", _uM("class" to "card-box"), _uA(
                            _cE("view", _uM("class" to "item", "onClick" to toOrder), _uA(
                                ...
                                _cV(_component_rice_icon, _uM("name" to "arrow-right", "size" to "20rpx"))
                            )),
                        ...
                        if (isTrue(isWechatEnv.value)) {
                            _cE("view", _uM("key" to 0, "class" to "btn-box"), _uA(
                                if (isTrue(isLoginState.value)) {
                                    _cV(_component_rice_button, _uM("key" to 0, "type" to "primary", "width" to "100%", "shape" to "round", "onClick" to handleLogin), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                        return _uA(
                                            "登录"
                                        )
                                    }), "_" to 1))
                                } else {
                                    if (isTrue(unref(getStorageSync)("usePhoneNumber"))) {
                                        _cV(_component_rice_button, _uM("key" to 1, "type" to "error", "width" to "100%", "shape" to "round", "onClick" to handleLogout), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                            return _uA(
                                                "退出登录"
                                            )
                                        }), "_" to 1))

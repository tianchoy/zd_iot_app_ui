...
open class GenPagesMyPkgMyPkg : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesMyPkgMyPkg) -> Any? = fun(__props): Any? {
            ...
            val pkgTabs = ref(_uA<PackageTab>(PackageTab(name = "全部"), PackageTab(name = "在用套餐"), PackageTab(name = "待生效"), PackageTab(name = "已失效")))
            ...
            val filteredPackages = computed<UTSArray<Any>>(fun(): UTSArray<Any> {
                val currentTab = pkgTabs.value[current__1.value]
                val currentStatus = if (currentTab == null) {
                    "全部"
                } else {
                    currentTab.name
                }
            ...
            val handlePkgDetail = fun(item: Any){
                uni_navigateTo(NavigateToOptions(url = "/pages/orderDetail/orderDetail?item=" + JSON.stringify(item)))
            }

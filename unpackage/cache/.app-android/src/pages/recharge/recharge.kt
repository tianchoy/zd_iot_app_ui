...
open class GenPagesRechargeRecharge : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesRechargeRecharge) -> Any? = fun(__props): Any? {
            ...
            val progress1 = ref(80)
            val active = ref(0)
            val tabs = ref(_uA(
                _uO("name" to "套餐包"),
                _uO("name" to "加油包")
            ))
            val packageList = ref(_uA(
                _uO("name" to "车联网月包20G", "tag" to "推荐", "data" to "20GB", "validity" to "30天", "price" to 90, "originalPrice" to 100),
                _uO("name" to "车联网月包10G", "tag" to "", "data" to "10GB", "validity" to "30天", "price" to 50, "originalPrice" to 0),
                _uO("name" to "工业设备月包5G", "tag" to "", "data" to "5GB", "validity" to "30天", "price" to 30, "originalPrice" to 0)
            ))
            val refillList = ref(_uA(
                _uO("name" to "加油包5G", "tag" to "推荐", "data" to "5GB", "validity" to "7天", "price" to 20, "originalPrice" to 30),
                _uO("name" to "加油包10G", "tag" to "", "data" to "10GB", "validity" to "15天", "price" to 35, "originalPrice" to 45),
                _uO("name" to "加油包20G", "tag" to "", "data" to "20GB", "validity" to "30天", "price" to 60, "originalPrice" to 80)
            ))
            val selectedPackageIndex = ref(0)
            val selectedRefillIndex = ref(0)
            val currentPrice = computed(fun(): Number {
                if (active.value === 0) {

...
function setTabBarItems(tabbar : string[] | null) {
	...
	tabbar.forEach((text, index) => {
		uni.setTabBarItem({
			...
			// success() {},
			fail(err) {
				__f__('warn','at uni_modules/lime-i18n/common/composer-class.uts:102',err.errMsg)
			}
	...
	__modifiers.forEach((value, key) => {
		if (typeof value == 'function') {
			try {
				...
				__f__('warn','at uni_modules/lime-i18n/common/composer-class.uts:187',35, '自定义修饰器函数必须是类型：(str: string) => string')
			}
		}
	})
	return _modifiers
		...
		if (typeof value == 'function') {
			try {
				...
				if (process.env.NODE_ENV !== 'production') {
					__f__('warn','at uni_modules/lime-i18n/common/composer-class.uts:210',35, '自定义复数化规则函数必须是类型: ( choice: number, choicesLength: number) => number')
				}
			}
		}
	})
...
export class ComposerClass {
	...
	private _initMethods() : void {
		...
		this._link = (str : string, locale : string | null, values : any, visitedLinkStack : string[], interpolateMode : string) : string | null => {
			...
			for (let i = 0; i < matches.length; i++) {
				...
				if (visitedLinkStack.includes(linkPlaceholder)) {
					__f__('warn','at uni_modules/lime-i18n/common/composer-class.uts:396',`发现循环引用。"${link}"已经在link"已经在${visitedLinkStack.reverse().join(' <- ')}链中访问过`);
					return ret;
				}
		...
		this._warnDefault = (key : string, message : string | null, values : any, interpolateMode : string) : string | null => {
			// 如果没有翻译，显示警告
			if (message == null) {
				__f__('warn','at uni_modules/lime-i18n/common/composer-class.uts:483',`无法翻译键路径 '${key}'. ` + '使用键路径的值作为默认值.');
			}
	...
	d(date : any, key : string | null = null, locale : string | null = null, options : UTSJSONObject | null = null) : string {
		...
		if (!this.availabilities.dateTimeFormat) {
			__f__('warn','at uni_modules/lime-i18n/common/composer-class.uts:604','无法格式化日期值，因为不支持 Intl.DateTimeFormat. ' + `key: ${key}, locale: ${locale}, options: ${options}`);
			return `${date}`;
		}
	...
	n(number : number, key : string | null = null, locale : string | null = null, options : UTSJSONObject | null = null) : string {
		...
		if (!this.availabilities.numberFormat) {
			__f__('warn','at uni_modules/lime-i18n/common/composer-class.uts:649','无法格式化数字值，因为不支持 Intl.NumberFormat. ' + `key: ${key}, locale: ${locale}, options: ${options}`);

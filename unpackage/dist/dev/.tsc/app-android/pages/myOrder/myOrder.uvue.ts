...
import {queryOrderList} from '@/api/http'
	...
	// Tab 列表
	type OrderStatusTab = { __$originalPosition?: UTSSourceMapPosition<"OrderStatusTab", "pages/myOrder/myOrder.uvue", 85, 7>;
		name: OrderStatus,
		value: string
	}
                  ...
                  _cE("view", _uM({ class: "order-details" }), [
                    ...
                    isTrue(order.id)
                      ? _cE("view", _uM({
                          ...
                          _cE("text", _uM({ class: "detail-label" }), "订单Id"),
                          _cE("text", _uM({ class: "detail-value" }), _tD(order.id || '-'), 1 /* TEXT */)
                        ])
                      : _cC("v-if", true),
                    _cE("view", _uM({ class: "detail-row last" }), [
                      _cE("text", _uM({ class: "detail-value" }), _tD(order.createTime || '-'), 1 /* TEXT */),
                      _cE("view", _uM({ class: "order-footer" }), [
                        ...
                        order.status === '0'
                          ? _cE("text", _uM({
                              key: 0,
                              class: "pay-btn",
                              onClick: withModifiers(() => {handlePay(order)}, ["stop"])
                            }), "去支付", 8 /* PROPS */, ["onClick"])
                          : _cC("v-if", true)
                      ])
                    ])

...
const _cache = __ins.renderCache;
	...
	const getLogin = async (code: string) => {
		...
		if(res.code == 200){
			console.log('登录成功:', res.data.access_token, " at pages/login/login.uvue:34")
	...
	onLoad((options: OnLoadOptions) => {
		console.log('登录参数:', options, " at pages/login/login.uvue:49")
...
const _component_rice_divider = resolveEasyComponent("rice-divider",_easycom_rice_divider)
  ...
  return _cE(Fragment, null, [
    ...
    _cE("view", _uM({ class: "container" }), [
      ...
      _cV(_component_rice_divider, _uM({ backgroundColor: "#f1f5f9" })),
      _cE("view", _uM({ class: "no-login" }), [
        _cE("view", _uM({ class: "no-login-text" }), "暂不登录")
      ])
    ])
  ], 64 /* STABLE_FRAGMENT */)
}

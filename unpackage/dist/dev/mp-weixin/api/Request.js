...
var _a$1;
let ApiResponse$1 = class ApiResponse extends common_vendor.UTS.UTSType {
...
const systemInfo$1 = common_vendor.index.getSystemInfoSync();
const DEFAULT_LANGUAGE$1 = ((_a$1 = systemInfo$1.language) !== null && _a$1 !== void 0 ? _a$1 : "zh_CN").replace("-", "_");
common_config.setStorageSync("uVueI18nLocale", systemInfo$1.language.replace("_", "-"));
let RequestOptions$1 = class RequestOptions extends common_vendor.UTS.UTSType {
...
let QuickRequestParams$1 = class QuickRequestParams extends common_vendor.UTS.UTSType {
  static get$UTSMetadata$() {
    return {
      kind: 2,
      get fields() {
        return {
          url: { type: String, optional: false },
          data: { type: "Unknown", optional: true },
          options: { type: RequestOptions$1, optional: true }
        };
      },
...
const DEFAULT_SUCCESS_CODES$1 = [0, 200];
const DEFAULT_UNAUTHORIZED_CODES$1 = [401, 403];
const DEFAULT_LOGIN_PAGE$1 = "/pages/card/card";
function buildUrl$1(url, baseUrl) {
  if (url.startsWith("http://") || url.startsWith("https://")) {
    return url;
  }
  return baseUrl + (url.startsWith("/") ? "" : "/") + url;
}
function buildQueryString$1(data) {
  if (!data)
    return "";
  const parts = [];
...
function showLoadingModal$1(text) {
  common_vendor.index.showLoading(new common_vendor.UTSJSONObject({ title: text, mask: true }));
}
function hideLoadingModal$1() {
  common_vendor.index.hideLoading();
}
function showErrorToast$1(msg) {
  common_vendor.index.showToast({ title: msg || "请求失败", icon: "none" });
}
let isRedirectingToLogin$1 = false;
function navigateToLogin$1(loginPage) {
  if (isRedirectingToLogin$1) {
    return null;
  }
  isRedirectingToLogin$1 = true;
  ...
  setTimeout(() => {
    ...
    isRedirectingToLogin$1 = false;
  }, 1e3);
}
function createRequestOptions$1(url, method, data = null, options = null) {
  const out = new RequestOptions$1({
    header: null,
    baseUrl: null,
    timeout: null,
    withToken: null,
...
function copyRequestOptions$1(options) {
  var _a2;
  return createRequestOptions$1(options.url, (_a2 = options.method) !== null && _a2 !== void 0 ? _a2 : "GET", options.data, options);
}
function request$1(options) {
  const url = options.url, _a2 = options.method, method = _a2 == void 0 ? "GET" : _a2, data = options.data;
  options.header;
  const baseUrl = options.baseUrl, _b = options.timeout, timeout = _b == void 0 ? DEFAULT_TIMEOUT$1 : _b, _c = options.withToken, withToken = _c == void 0 ? false : _c, _d = options.showError, showError = _d == void 0 ? true : _d, _e = options.showLoading, showLoading = _e == void 0 ? false : _e, loadingText = options.loadingText, _f = options.redirectOnUnauthorized, redirectOnUnauthorized = _f == void 0 ? true : _f, loginPage = options.loginPage, successCodes = options.successCodes, unauthorizedCodes = options.unauthorizedCodes, onErrorCode = options.onErrorCode;
  if (showLoading) {
    showLoadingModal$1(loadingText !== null && loadingText !== void 0 ? loadingText : "加载中...");
  }
  const base = baseUrl !== null && baseUrl !== void 0 ? baseUrl : api_ProjectConfig.getHostProjectConfig().baseUrl;
  let fullUrl = buildUrl$1(url, base);
  let requestData = data;
  if (method === "GET" && data != null) {
    fullUrl += buildQueryString$1(data);
  ...
  const reqHeader = new common_vendor.UTSJSONObject(
    {
      ...
      "Content-Language": DEFAULT_LANGUAGE$1,
  ...
  const finalSuccessCodes = successCodes !== null && successCodes !== void 0 ? successCodes : DEFAULT_SUCCESS_CODES$1;
  const finalUnauthorizedCodes = unauthorizedCodes !== null && unauthorizedCodes !== void 0 ? unauthorizedCodes : DEFAULT_UNAUTHORIZED_CODES$1;
  const hpLogin = api_ProjectConfig.getHostProjectConfig().loginPagePath;
  const finalLoginPage = loginPage !== null && loginPage !== void 0 ? loginPage : hpLogin.length > 0 ? hpLogin : DEFAULT_LOGIN_PAGE$1;
  return new Promise((resolve, reject) => {
    common_vendor.index.request({
      ...
      success: (res) => {
        ...
        const result = new ApiResponse$1({
          code,
        ...
        if (common_config.isWechat()) {
          ...
          if (isUnauthorizedCode && redirectOnUnauthorized) {
            if (showError) {
              showErrorToast$1(msg || "登录已过期，请重新登录");
            }
            navigateToLogin$1(finalLoginPage);
            reject(result);
            return null;
          }
        }
      ...
      fail: (err) => {
        ...
        if (showError) {
          showErrorToast$1("网络异常，请检查网络连接");
        }
        reject(new ApiResponse$1({
          rows: null,
          total: null,
          code: -1,
          msg: "网络异常",
...
function get$1(url, data = null, options = null) {
  return request$1(createRequestOptions$1(url, "GET", data, options));
}
function post$1(url, data = null, options = null) {
  return request$1(createRequestOptions$1(url, "POST", data, options));
}
function put$1(url, data = null, options = null) {
  return request$1(createRequestOptions$1(url, "PUT", data, options));
}
function del$1(url, data = null, options = null) {
  return request$1(createRequestOptions$1(url, "DELETE", data, options));
}
function publicRequest$1(options) {
  const requestOptions = copyRequestOptions$1(options);
  requestOptions.withToken = false;
  requestOptions.redirectOnUnauthorized = false;
  return request$1(requestOptions);
}
function silentRequest$1(options) {
  const requestOptions = copyRequestOptions$1(options);
  requestOptions.showError = false;
  requestOptions.redirectOnUnauthorized = false;
  return request$1(requestOptions);
}
function loadingRequest$1(options, loadingText = null) {
  const requestOptions = copyRequestOptions$1(options);
  requestOptions.showLoading = true;
  requestOptions.loadingText = loadingText || "加载中...";
  return request$1(requestOptions);
}
const http$1 = new common_vendor.UTSJSONObject(
  {
    /** GET 请求 */
    get(url, data = null, options = null) {
      return get$1(url, data, options);
    },
    /** POST 请求 */
    post(url, data = null, options = null) {
      return post$1(url, data, options);
    },
    /** PUT 请求 */
    put(url, data = null, options = null) {
      return put$1(url, data, options);
    },
    /** DELETE 请求 */
    delete(url, data = null, options = null) {
      return del$1(url, data, options);
    },
    /** 公开请求（不需要登录） */
    public(options) {
      return publicRequest$1(options);
    },
    /** 静默请求（不显示提示） */
    silent(options) {
      return silentRequest$1(options);
    },
    /** 带加载提示的请求 */
    loading(options, loadingText = null) {
      return loadingRequest$1(options, loadingText);
    },
...
new common_vendor.UTSJSONObject({
  request: request$1,
  public: publicRequest$1,
  silent: silentRequest$1,
  loading: loadingRequest$1,
  http: http$1
});
var _a;
class ApiResponse2 extends common_vendor.UTS.UTSType {
  static get$UTSMetadata$(T) {
    return {
      kind: 2,
      get fields() {
...
class RequestOptions2 extends common_vendor.UTS.UTSType {
  static get$UTSMetadata$() {
    return {
      kind: 2,
      get fields() {
  ...
  constructor(options, metadata = RequestOptions2.get$UTSMetadata$(), isJSONParse = false) {
    super();
    this.__props__ = common_vendor.UTS.UTSType.initProps(options, metadata, isJSONParse);
    this.url = this.__props__.url;
    this.method = this.__props__.method;
...
class QuickRequestParams2 extends common_vendor.UTS.UTSType {
  static get$UTSMetadata$() {
    return {
      kind: 2,
      get fields() {
        return {
          url: { type: String, optional: false },
          data: { type: "Unknown", optional: true },
          options: { type: RequestOptions2, optional: true }
        };
      },
      name: "QuickRequestParams"
    };
  }
  constructor(options, metadata = QuickRequestParams2.get$UTSMetadata$(), isJSONParse = false) {
    super();
    this.__props__ = common_vendor.UTS.UTSType.initProps(options, metadata, isJSONParse);
    this.url = this.__props__.url;
    this.data = this.__props__.data;
...
exports.RequestOptions = RequestOptions2;
exports.http = http$1;
